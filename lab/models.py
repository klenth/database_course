...
class Student(Person):
    ...
    def attempts(self, problem):
        return ProblemAttempt.objects.filter(student=self, problem=problem)
...
class ProblemSchema(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid1)
    file = models.FileField(upload_to=f'{settings.UPLOAD_SCHEMA_DIR}/%Y/%m/%d/', null=True, default=None)
...
class ProblemTableData(models.Model):
    ...
    data_file = models.FileField(upload_to=f'{settings.UPLOAD_TABLE_DATA_DIR}/%Y/%m/%d/', null=False)
...
class ProblemTestCase(models.Model):
    ...
    result_data_file = models.FileField(upload_to=f'{settings.UPLOAD_CORRECT_RESULT_DIR}/%Y/%m/%d/', null=True, default=None)
...
class AttemptResults(models.Model):
    ...
    data_file = models.FileField(upload_to=f'{settings.UPLOAD_ATTEMPT_RESULT_DIR}/%Y/%m/%d/', null=False)
    score = models.PositiveSmallIntegerField()
